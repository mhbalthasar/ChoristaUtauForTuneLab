...
namespace OpenUtau.Core {
    public class TimeAxis {
        ...
        public virtual void BuildSegments(UProject project) {
        ...
        public virtual double GetBpmAtTick(int tick) {
            var segment = tempoSegments.First(seg => seg.tickPos == tick || seg.tickEnd > tick); // TODO: optimize
            return segment.bpm;
        }

        public virtual double TickPosToMsPos(double tick) {
            var segment = tempoSegments.First(seg => seg.tickPos == tick || seg.tickEnd > tick); // TODO: optimize
            return segment.msPos + segment.msPerTick * (tick - segment.tickPos);
        }

        public virtual double MsPosToNonExactTickPos(double ms) {
        ...
        public virtual int MsPosToTickPos(double ms) {
            var segment = tempoSegments.First(seg => seg.msPos == ms || seg.msEnd > ms); // TODO: optimize
            double tickPos = segment.tickPos + (ms - segment.msPos) * segment.ticksPerMs;
            return (int)Math.Round(tickPos);
        }

        public virtual int TicksBetweenMsPos(double msPos, double msEnd) {
            return MsPosToTickPos(msEnd) - MsPosToTickPos(msPos);
        }

        public virtual double MsBetweenTickPos(double tickPos, double tickEnd) {
        ...
        public virtual int MsToTickAt(double offsetMs, int refTickPos) {
            return TicksBetweenMsPos(
                TickPosToMsPos(refTickPos), 
                TickPosToMsPos(refTickPos) + offsetMs);
        }

        public virtual void TickPosToBarBeat(int tick, out int bar, out int beat, out int remainingTicks) {
        ...
        public virtual int BarBeatToTickPos(int bar, int beat) {
        ...
        public virtual void NextBarBeat(int bar, int beat, out int nextBar, out int nextBeat) {
            nextBar = bar;
            nextBeat = beat + 1;
            var segment = timeSigSegments.First(seg => seg.barPos == bar || seg.barEnd > bar); // TODO: optimize
        ...
        public virtual UTempo[] TemposBetweenTicks(int start, int end) {
        ...
        public virtual UTimeSignature TimeSignatureAtTick(int tick) {
            var segment = timeSigSegments.First(seg => seg.tickPos == tick || seg.tickEnd > tick); // TODO: optimize
        ...
        public virtual UTimeSignature TimeSignatureAtBar(int bar) {
            var segment = timeSigSegments.First(seg => seg.barPos == bar || seg.barEnd > bar); // TODO: optimize
            return new UTimeSignature {
                barPosition = segment.barPos,
                beatPerBar = segment.beatPerBar,
